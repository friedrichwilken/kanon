//! Embeddings for the `dense` and `hybrid` backends.
//!
...
pub enum EmbedError {
    ...
    /// `KANON_EMBED_URL` was not set for `eval --backend dense/hybrid` (a distinct variant
    /// from [`EmbedError::MissingConfig`] because its wording predates this one and several
    /// tests pin it verbatim).
    #[error("KANON_EMBED_URL is not set (needed for --backend dense/hybrid)")]
    MissingEmbedUrl,
...
/// A source of text embeddings, injectable so tests avoid the network.
pub trait Embedder {
...
impl HttpEmbedder {
    ...
    /// Build a client and resolve the model name from `KANON_EMBED_URL`, `KANON_EMBED_KEY`
    /// and `KANON_EMBED_MODEL`, with `model_arg` (`--model`) overriding the model env var.
    ...
    pub fn from_env(model_arg: Option<&str>) -> Result<(HttpEmbedder, String), EmbedError> {
        let env = crate::env::var;
        let base_url = env("KANON_EMBED_URL")
            .ok_or_else(|| EmbedError::MissingConfig("KANON_EMBED_URL".to_string()))?;
        let api_key = env("KANON_EMBED_KEY");
        let model = model_arg
            .map(str::to_string)
            .or_else(|| env("KANON_EMBED_MODEL"))
            .ok_or_else(|| {
                EmbedError::MissingConfig("KANON_EMBED_MODEL (or --model)".to_string())
            })?;
...
/// `embeddings.json`: what `embeddings.bin` holds and how it was produced.
...
pub fn artifact_manifest_hash(artifact: &Path) -> Result<String, EmbedError> {
    let path = artifact.join(pinakes::layout::MANIFEST_FILE);
    match std::fs::read(&path) {
        Ok(bytes) => Ok(pinakes::text::sha256_hex(&bytes)),
...
pub mod testing {
    ...
    fn embed_one(text: &str) -> Vec<f32> {
        ...
        for token in pinakes::index::tokenize(text) {
...
mod tests {
    ...
    fn artifact_hash_falls_back_to_none() {
        ...
        assert_eq!(hash, pinakes::text::sha256_hex(b"{}"));
    }
